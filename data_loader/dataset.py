...
class Dataset():
    ...
    def get_data(self):
        ...
        if self.for_what == "training_source":
            for image in os.listdir(self.train_folder):
                if image == "source":
                    xs_train_img_folder = os.path.join(
                        self.train_folder, image)
                    for img in os.listdir(xs_train_img_folder):
                        xs_train_img_path = os.path.join(
                            xs_train_img_folder, img)
                        self.xs_train_img_files.append(xs_train_img_path)
                        # print(f"training images:{self.train_img_files}")
                elif image == "source_labels":
                    ys_train_labels_folder = os.path.join(
                        self.train_folder, image)
                    for lab in os.listdir(ys_train_labels_folder):
                        ys_train_label_path = os.path.join(
                            ys_train_labels_folder, lab)
        ...
        if self.for_what == "training_target":
            for image in os.listdir(self.train_folder):
                if image == "target":
                    xt_train_img_folder = os.path.join(
                        self.train_folder, image)
                    for img in os.listdir(xt_train_img_folder):
                        xt_train_img_path = os.path.join(
                            xt_train_img_folder, img)
                        self.xt_train_img_files.append(xt_train_img_path)
                        # print(f"training images:{self.train_img_files}")
                elif image == "target_labels":
                    yt_train_labels_folder = os.path.join(
                        self.train_folder, image)
                    for lab in os.listdir(yt_train_labels_folder):
                        yt_train_label_path = os.path.join(
                            yt_train_labels_folder, lab)
        ...
        if self.for_what == "training_source":
            ...
            if os.path.exists(self.train_folder + "/source_img_patches/"):
                ...
                for i in os.listdir(self.train_folder + "/source_img_patches/"):
                    im = rasterio.open(self.train_folder +
                                       "/source_img_patches/"+i).read()
                    self.xs_train_patches.append(im)
                self.Xs_train = np.asarray(self.xs_train_patches)
                print("shape of previously patched images are:",
                      self.Xs_train.shape)
                ...
                self.xs_train_patches = Patch.image_patching(
                    self, self.Xs_train_main)
                # normalize
                self.Xs_train_norm = []
                for i in range(len(self.xs_train_patches)):
                    self.norm_im = PreProcess.bytes8caling(
                        self, self.xs_train_patches[i, :, :, :], chanell_order="first")
                    self.Xs_train_norm.append(self.norm_im)
                Xs_train_ax = np.asarray(self.Xs_train_norm)
                self.Xs_train = np.moveaxis(Xs_train_ax, 3, 1)
                print(
                    f"shape of normalized source images are {self.Xs_train.shape}")
                ...
                print(
                    "!!!!!!!!!!!! saving source images as npy after patched and normalized !!!!!!!!!!!!!!")
            ...
            if os.path.exists(self.train_folder + "/source_label_patches/"):
                ...
                self.ys_train_patches = Patch.image_patching(
                    self, self.Ys_train_main)
            ...
            if os.path.exists(self.train_folder + "/target_img_patches/"):
                ...
                print(
                    "---------------it was only for target that patched images are loaded then normalized----------")
                # normalize
                Xt_train_norm = []
                for i in range(len(self.Xt_train_main)):
                    self.norm_imt = PreProcess.bytes8caling(
                        self, self.Xt_train_main[i, :, :, :], chanell_order="first")
                    Xt_train_norm.append(self.norm_imt)
                Xt_train_ax = np.asarray(Xt_train_norm)
                self.Xt_train = np.moveaxis(Xt_train_ax, 3, 1)
                print(
                    f"shape of normalized target images are {self.Xt_train.shape}")
            ...
            if os.path.exists(self.test_folder + "/testing_images/"):
                ...
                for i in os.listdir(self.test_folder + "/testing_images/"):
                    im = rasterio.open(self.test_folder +
                                       "/testing_images/"+i).read()
    ...
    def array_torch(self):
        ...
        if self.for_what == "both":
            ...
            self.tensor_xs_train = TensorDataset(
                self.tensor_xs, self.tensor_ys)

            # split into train and validation set
            sn_train_examples = int(
                len(self.tensor_xs_train) * config["valid_ratio"])
            sn_valid_examples = len(self.tensor_xs_train) - sn_train_examples
            self.source_train_partly, self.source_valid_partly = random_split(
                self.tensor_xs_train, [sn_train_examples, sn_valid_examples])
            print("--------------------------------------------------------------------")
            print(
                f'Number of source training examples: {len(self.source_train_partly)}')
            print(
                f'Number of source validation examples: {len(self.source_valid_partly)}')
            # create dataloader
            self.source_dataloader = DataLoader(
                self.source_train_partly, batch_size=config["batchsize"], shuffle=True, pin_memory=True, worker_init_fn=np.random.seed(42))
            self.valid_source_dataloader = DataLoader(
                self.source_valid_partly, batch_size=config["batchsize"], shuffle=False, pin_memory=True, worker_init_fn=np.random.seed(42))
            ...
            self.tensor_xt_train = TensorDataset(
                self.tensor_xt, self.tensor_yt)
            # split into train and validation set
            tn_train_examples = int(
                len(self.tensor_xt_train) * config["valid_ratio"])
            tn_valid_examples = len(self.tensor_xt_train) - tn_train_examples
            self.target_train_partly, self.target_valid_partly = random_split(
                self.tensor_xt_train, [tn_train_examples, tn_valid_examples])
            print("--------------------------------------------------------------------")
            print(
                f'Number of target training examples: {len(self.target_train_partly)}')
            print(
                f'Number of target validation examples: {len(self.target_valid_partly)}')
            # create dataloader
            self.target_dataloader = DataLoader(
                self.target_train_partly, batch_size=config["batchsize"], shuffle=True, pin_memory=True, worker_init_fn=np.random.seed(42))
            self.valid_target_dataloader = DataLoader(
                self.target_valid_partly, batch_size=config["batchsize"], shuffle=False, pin_memory=True, worker_init_fn=np.random.seed(42))
            ...
            self.source_dataloader = DataLoader(
                self.source_train_partly, batch_size=config["batchsize"], shuffle=True, pin_memory=True, worker_init_fn=np.random.seed(42))
            self.valid_source_dataloader = DataLoader(
                self.source_valid_partly, batch_size=config["batchsize"], shuffle=True, pin_memory=True, worker_init_fn=np.random.seed(42))
            print("Finally atleast train and valid source dataloader section works 😌 ")
            ...
            # create dataloader
            self.target_dataloader = DataLoader(
                self.target_train_partly, batch_size=config["batchsize"], pin_memory=True, worker_init_fn=np.random.seed(42))
            self.valid_target_dataloader = DataLoader(
                self.target_valid_partly, batch_size=config["batchsize"], pin_memory=True, worker_init_fn=np.random.seed(42))
            print("Finally atleast train and valid target dataloader section works 😌 ")
            ...
            self.test_dataloader = DataLoader(
                self.tensor_test, batch_size=1, pin_memory=True, worker_init_fn=np.random.seed(0))
